...
export const addThing = assign({
	things: (context, { name }) => context.things.concat([{ id: uuid(), name, done: [] }])
});
...
export const removeThing = assign({
	things: (context) => context.things.filter((t) => t.id !== context.currentThing),
	currentThing: null
});
...
function undoSomeThing(things, id, index) {
	return editThing(things, id, (done) => done.slice(0, index).concat(done.slice(index + 1)));
}
...
export const undoThisThing = assign({
	things: (context, { index }) => undoSomeThing(context.things, context.currentThing, index)
});
