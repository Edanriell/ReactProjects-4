...
export function DataProvider({ children }) {
	...
	useEffect(
		() => localStorage.setItem(STORAGE_KEY, JSON.stringify({ things, currentThing })),
		[things, currentThing]
	);
	...
	const editThing = useCallback(
		(id, cb) => setThings((ts) => ts.map((t) => (t.id === id ? { ...t, done: cb(t.done) } : t))),
		[]
	);
	...
	const undoThing = useCallback(
		(id, index) => editThing(id, (done) => done.slice(0, index).concat(done.slice(index + 1))),
		[editThing]
	);
	const value = {
		state: {
