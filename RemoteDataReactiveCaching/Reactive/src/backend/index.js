...
function missingAuth() {
	if (!store.isAuthorized) {
		return delayedResponse(() => new HttpResponse("Unauthorized", { status: 403 }));
	}
}
...
export const worker = setupWorker(
	...
	http.post("/api/session", () => {
		...
		return delayedResponse(() => HttpResponse.json({ status: "SESSION_CREATED" }));
	}),
	// LOGOUT
	http.delete("/api/session", () => {
		updateStore({ isAuthorized: false });
		return delayedResponse(() => HttpResponse.json({ status: "SESSION_DELETED" }));
	}),
	// SIGNUP
	http.post("/api/user", () => {
		updateStore({ isAuthorized: true });
		return delayedResponse(() => HttpResponse.json({ status: "USER_CREATED" }));
	}),
	...
	http.get("/api/user", () => {
		...
		return delayedResponse(() => HttpResponse.json({ status: "USER_EXISTS" }));
	}),
	// GET ALL THINGS
	http.get("/api/things", () => {
		const invalid = missingAuth();
	...
	http.delete("/api/things/:tid", ({ params: { tid } }) => {
		...
		updateStore({ things });
		return delayedResponse(() => HttpResponse.json({ status: "THING_DELETED" }));
	}),
	// ADD A THING
	http.post("/api/things", async ({ request }) => {
		...
		const things = store.things.concat([{ id: uuid(), done: [], name, description }]);
		updateStore({ things });
		return delayedResponse(() => HttpResponse.json({ status: "THING_CREATED" }));
	}),
	...
	http.delete("/api/things/:tid/done/last", ({ params: { tid } }) => {
		...
		updateStore({ things });
		return delayedResponse(() => HttpResponse.json({ status: "DONE_DELETED" }));
	}),
	// DELETE DONE FOR THING
	http.delete("/api/things/:tid/done/:did", ({ params: { tid, did } }) => {
		const invalid = missingAuth();
		if (invalid) {
			return invalid;
		}
	...
	http.post("/api/things/:tid/done", ({ params: { tid } }) => {
		...
		updateStore({ things });
		return delayedResponse(() => HttpResponse.json({ status: "DONE_CREATED" }));
	})
);
