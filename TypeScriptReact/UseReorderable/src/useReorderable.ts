...
export function useReorderable<T>(initial: State<T>) {
	const [state, dispatch] = useReducer<Reducer<State<T>, Action>>(reorder, initial);

	return {
		list: state,
		moveUp: (index: number) => dispatch({ type: "moveUp", index }),
		moveDown: (index: number) => dispatch({ type: "moveDown", index }),
		moveToTop: (index: number) => dispatch({ type: "moveToTop", index }),
		moveToBottom: (index: number) => dispatch({ type: "moveToBottom", index })
	};
}
