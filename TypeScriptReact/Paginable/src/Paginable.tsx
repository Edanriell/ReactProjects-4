...
export function Paginable<Type>({ items, Renderer, className }: PaginableProps<Type>) {
	...
	const { subset, numPages, pages, startOffset } = useMemo(() => {
		const startOffset = currentPage * PERPAGE;
		const endOffset = (currentPage + 1) * PERPAGE;
		const subset = items.slice(startOffset, endOffset);
		const numPages = Math.ceil(items.length / PERPAGE);
		...
		return { subset, numPages, pages, startOffset };
	}, [items, currentPage]);
					...
					{pages.map((page) => (
						<li className="pagination__item" key={page}>
							{page === currentPage ? (
								...
								<button className="pagination__link" onClick={() => setCurrentPage(page)}>
									{page + 1}
								</button>
							)}
						</li>
